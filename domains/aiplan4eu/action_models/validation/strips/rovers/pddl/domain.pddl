; navigate action update to prevent navigating from and to the same waypoint
; communicate actions effects refined, available and channel_free where negative and positive at the same time
(define (domain Rover)
(:requirements :typing)
...
:precondition (and (can_traverse ?x ?y ?z) (available ?x) (at ?x ?y) (not (at ?x ?z))
                (visible ?y ?z)
	    )
