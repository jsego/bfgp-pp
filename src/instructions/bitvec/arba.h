...
#ifndef __INSTRUCTIONS_ARBA_H__
#define __INSTRUCTIONS_ARBA_H__
...
namespace instructions::bitvec {
    class Arba : public Instruction {
    public:
        Arba(id_type lhs_ptr_id, id_type rhs_ptr_id, std::shared_ptr<StateDescriptor> sd) {
            /// Arba instruction: it assumes there is a unique predicate type named "vector"
        ...
        [[nodiscard]] std::string to_string(bool full_info ) const override {
            std::string ret;
            if( full_info ) ret = "[INSTRUCTION][ARBA]:\n";
            return ret + _op_assign->get_lhs()->to_string(false) + " = arba(" +
