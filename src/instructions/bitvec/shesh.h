...
#ifndef __INSTRUCTIONS_SHESH_H__
#define __INSTRUCTIONS_SHESH_H__
...
namespace instructions::bitvec {
    class Shesh : public Instruction {
    public:
        Shesh(id_type lhs_ptr_id, id_type rhs_ptr_id, std::shared_ptr<StateDescriptor> sd) {
            /// Shesh instruction: it assumes there is a unique predicate type named "vector"
        ...
        [[nodiscard]] std::string to_string(bool full_info ) const override {
            std::string ret;
            if( full_info ) ret = "[INSTRUCTION][SHESH]:\n";
            return ret + _op_assign->get_lhs()->to_string(false) + " = shesh(" +
...
#endif //__INSTRUCTIONS_SHESH_H__
